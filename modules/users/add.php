            ...
            if ($existingEmail) {
                ...
                if ($db->query($sql, $params)) {
                    ...
                    $db->query(
                        ...
                        [$_SESSION['user_id'], 'create', 'user', $userId, "Created user: $username"]
                    );

                    setAlert('تم إضافة المستخدم بنجاح', 'success');
                    // Clear any output buffer before redirect
                    ob_end_clean();
                    header('Location: ' . SITE_URL . '/modules/users/view.php?id=' . $userId);
                    exit;
                } else {
            ...
            <form method="POST" class="needs-validation" novalidate>
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="invalid-feedback">يجب أن تتطابق كلمات المرور</div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="role" class="form-select" required>
                            <option value="staff" <?php echo (isset($_POST['role']) && $_POST['role'] === 'staff') ? 'selected' : ''; ?>>موظف</option>
                            <option value="lab_manager" <?php echo (isset($_POST['role']) && $_POST['role'] === 'lab_manager') ? 'selected' : ''; ?>>مدير مختبر</option>
                            <option value="faculty_manager" <?php echo (isset($_POST['role']) && $_POST['role'] === 'faculty_manager') ? 'selected' : ''; ?>>مدير كلية</option>
                            <option value="admin" <?php echo (isset($_POST['role']) && $_POST['role'] === 'admin') ? 'selected' : ''; ?>>مدير</option>
                            <option value="guest" <?php echo (isset($_POST['role']) && $_POST['role'] === 'guest') ? 'selected' : ''; ?>>زائر</option>
                        </select>
                    </div>
                        ...
                        <select name="faculty_id" class="form-select" id="faculty_id">
                            ...
                            <?php foreach ($faculties as $faculty): ?>
                                <option value="<?php echo $faculty['id']; ?>"
                                    <?php echo (isset($_POST['faculty_id']) && $_POST['faculty_id'] == $faculty['id']) ? 'selected' : ''; ?>>
